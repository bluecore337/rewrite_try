...
std::optional<PowerRune> Buff_Detector::detect(cv::Mat & bgr_img)
{
  ...
  // 分离 R 标（label=1）和 扇叶（label=0）
  // YOLO11_BUFF: class_names = {"buff", "r"}
  std::vector<FanBlade> fanblades;
  cv::Point2f r_pixel_center(0, 0);
  ...
  // 填充 R 标信息
  powerrune.has_r_mark = found_r;
  powerrune.r_center_pixel = r_pixel_center;
  powerrune.r_corners = r_corners;
