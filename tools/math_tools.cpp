...
Eigen::MatrixXd xyz2ypd_jacobian(const Eigen::Vector3d & xyz)
{
  ...
  auto x = xyz[0], y = xyz[1], z = xyz[2];
  auto xy2 = std::max(x * x + y * y, eps);
  auto xy = std::sqrt(xy2);
  auto xyz2 = std::max(x * x + y * y + z * z, eps);
  auto pitch_common = std::max(xy2 + z * z, eps);

  auto dyaw_dx = -y / xy2;
  auto dyaw_dy = x / xy2;
  auto dyaw_dz = 0.0;

  auto dpitch_dx = -(x * z) / (xy * pitch_common);
  auto dpitch_dy = -(y * z) / (xy * pitch_common);
  auto dpitch_dz = xy / pitch_common;

  auto distance = std::sqrt(xyz2);
  auto ddistance_dx = x / distance;
  auto ddistance_dy = y / distance;
  auto ddistance_dz = z / distance;
